...
begin
  ...
  set company_name = nullif(trim(target_company_name), ''),
      contact_name = nullif(trim(target_contact_name), ''),
      email = lower(nullif(trim(target_email), '')),
      phone = nullif(trim(coalesce(target_phone, '')), '')
  where id = target_client_id
  returning * into client_record;

  if client_record.company_name is null
    or client_record.contact_name is null
    or client_record.email is null then
...
begin
  ...
  recipient_id := coalesce(draft_record.created_by, draft_record.assigned_to);
  ...
  begin
    ...
    values (
      ...
      jsonb_build_object(
        ...
        'recipient_policy', 'schedule_creator_only'
      ),
      dedupe,
      false
    )
...
declare
  ...
  recipient_id uuid;
begin
  recipient_id := coalesce(p_draft.created_by, p_draft.assigned_to);
  if recipient_id is null then
    return;
  end if;
  ...
  values (
    ...
    resolved_priority,
    'content_review',
    'content-review:' || p_draft.id || ':' || p_dedupe_suffix || ':' || recipient_id,
    false,
    jsonb_build_object(
      'draftId', p_draft.id,
      'recipient_policy', 'schedule_creator_only'
