create or replace function public.is_content_studio_office(target_office_id uuid)
returns boolean
language sql
security definer
...
as $$
  select exists (
    ...
    where co.id = target_office_id
      and coalesce(nullif(trim(co.slug), ''), '') <> 'three-little-birds'
  );
$$;

grant execute on function public.is_content_studio_office(uuid) to authenticated, anon;
...
as $$
  select exists (
    ...
    where p.id = auth.uid()
      ...
      and public.is_content_studio_office(target_office_id)
...
-- Notify staff on the same office as the draft (any Content Studio–eligible office).
    ...
    (
      ...
      from (
        ...
        where p.organization_id = draft_record.organization_id
          and p.office_id = draft_record.office_id
          and public.is_content_studio_office(p.office_id)
          and p.primary_role::text in (
            'admin',
            'org_admin',
            'super_admin',
...
begin
  ...
  from (
    ...
    where p.organization_id = p_draft.organization_id
      and p.office_id = p_draft.office_id
      and public.is_content_studio_office(p.office_id)
      and p.primary_role::text in (
        'admin',
        'org_admin',
